...
def get_all():
    ...
    if amount_gte > 0:
        payments = Payment.query.filter(Payment.amount >= amount_gte).paginate(
            page=page, per_page=per_page
        )
    elif amount_lte > 0:
        payments = Payment.query.filter(Payment.amount <= amount_lte).paginate(
            page=page, per_page=per_page
        )
    elif invoice_id > 0:
        payments = Payment.query.filter_by(customer_id=customer_id).paginate(
            page=page, per_page=per_page
        )
